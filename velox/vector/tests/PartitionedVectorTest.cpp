...
// A vector appearing under more than one column must be partitioned exactly
// once. The in-place scatter permutes the vector's buffers, so a second
// partition() call on the same object would double-permute both columns.
