...
// Verifies replicateNullsAndAny: rows with null partition keys and one
// arbitrary row are delivered to every destination, while the remaining rows
// are delivered to their hash partition only.
TEST_F(OptimizedPartitionedOutputTest, replicateNullsAndAny) {
  ...
  for (int p = 0; p < kNumPartitions; ++p) {
    futures.push_back(std::async(
        std::launch::async, [&, p] { return getAllData(taskId, p); }));
  }
  ...
  for (int p = 0; p < kNumPartitions; ++p) {
    ...
    // The null-key rows ("b", "d") and the replicated "any" row ("a", the
    // first row seen) reach every destination exactly once.
  ...
  // Rows with non-null keys other than the "any" row are delivered to their
  // hash partition only.
