...
TEST_F(MapUnionSumTest, global) {
  auto data = makeRowVector({
      makeNullableMapVector<int64_t, int64_t>({
          std::make_optional<std::vector<std::pair<int64_t, std::optional<int64_t>>>>({}), // empty map
...
TEST_F(MapUnionSumTest, globalVarcharKey) {
  ...
  auto data = makeRowVector({
      makeNullableMapVector<StringView, int64_t>({
          std::make_optional<std::vector<std::pair<StringView, std::optional<int64_t>>>>({}), // empty map
          std::nullopt, // null map
          {{{keys[0], 10}, {keys[1], 20}}},
